...
public class ImageViewer extends Core {
    ...
    public void updateImage(boolean changedSettings){
            ...
            synchronized (loadImageLock){
                if (isAnimated) {
                    if (changedSettings || animatedRunnable == null || preProcess==null) {
                        ...
                        animatedRunnable = new ImmediatelyCancellableBukkitRunnable() {
                            ...
                            public void run() {
                                ...
                                while (!this.isCancelled()) {
                                    ...
                                    Utils.Pair<Rectangle2D.Float,byte[]> preData;
                                    synchronized (loadImageLock)
                                    {
                                        if(isAnimated){
                                            ...
                                            preData = new Utils.Pair<>(pre[nowFrame].getKey(),pre[nowFrame].getValue());
                                            nowFrame++;
                                        }else{
                                            animatedRunnable=null;
                                            return;
